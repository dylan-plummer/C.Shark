...
class TrainModule(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.input_pred_model = ModelClass(
            num_genomic_features=2, # Input features
            num_target_tracks=3,    # Target 1D tracks
...
def main():
    ...
    with torch.no_grad():
        for start, end in tqdm(zip(starts, ends), total=len(starts), desc="Processing Windows"):
            ...
            ctcf_ko_region = knockout_peaks(ctcf_region.copy(), threshold=0.5)
            # Feed to model (Model infers RAD21 from zeroed CTCF KO)
            inputs_ko = infer.preprocess_default(seq_region, ctcf_ko_region, atac_region).to(device)
            pred_ko_all = inner_model(inputs_ko)['1d']
            pred_ko_rad21 = torch.expm1(pred_ko_all[:, :, rad21_idx]).squeeze()
            ...
            full_inputs_with_pred_rad21 = infer.preprocess_default(seq_region, ctcf_ko_region, atac_region, other_feats).to(device)
            # Replace RAD21 channel (assumed to be after seq, ctcf, atac channels)
            full_inputs_with_pred_rad21[:,:,5 + rad21_idx] = pred_ko_rad21
            output_ko = model(full_inputs_with_pred_rad21)
            ...
            if np.random.rand() < 0.2:
                min_99p_val = 0.5
                if np.percentile(pred_hic_wt, 99) < min_99p_val or np.percentile(gt_hic_wt, 99) < min_99p_val or \
                   np.percentile(pred_hic_ko, 99) < min_99p_val or np.percentile(gt_hic_ko, 99) < min_99p_val:
                    pass
                else:
    ...
    output_df['anchor1'] = (output_df['start1'] // args.resolution).astype(str)
    output_df['anchor2'] = (output_df['start2'] // args.resolution).astype(str)
