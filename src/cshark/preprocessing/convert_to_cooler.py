...
if __name__ == '__main__':
    ...
    parser.add_argument('--anchors', type=str, help='directory containing chromosome .bed files')
    ...
    for file in tqdm(sorted_nicely(chrom_files)):
        ...
        if single_chrom:
            if single_chrom_name != chr_name:
                continue
        if os.path.isdir(anchor_dir):
            anchor_file = os.path.join(anchor_dir, chr_name + '.bed')
        else:
            anchor_file = anchor_dir
        anchor_list = pd.read_csv(anchor_file, sep='\t', usecols=[0, 1, 2, 3], names=['chrom', 'start', 'end', 'anchor'])  # read anchor list file
        print(anchor_list)
        # filter to chromosome
        bins = anchor_list[anchor_list['chrom'] == chr_name].reset_index(drop=True)
        #bin_file = os.path.join(anchor_dir, chr_name + '.bed')
        ...
        if force_bin_size:
            ...
            pixels['new_bin1'] = pixels.apply(anchor_to_bin(anchor_map, key='a1', offset=chr_bin_offset), axis=1)
            print_update('Computing original row lengths...', verbose=verbose)
            pixels['bin1_length'] = pixels.apply(get_anchor_length(anchor_map, key='a1'), axis=1)
            print_update('Finding new col IDs...', verbose=verbose)
            pixels['new_bin2'] = pixels.apply(anchor_to_bin(anchor_map, key='a2', offset=chr_bin_offset), axis=1)
            print_update('Computing original col lengths...', verbose=verbose)
            pixels['bin2_length'] = pixels.apply(get_anchor_length(anchor_map, key='a2'), axis=1)
