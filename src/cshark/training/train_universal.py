        ...
        for celltype in self.celltypes:
            for chr_name, start in zip(self.chr_names, self.starts):
                try:
                    ...
                    pred_1d_tracks = outputs.get('1d')
                    if pred_1d_tracks is not None:
                        os.makedirs(os.path.join(self.out_dir, locus, celltype, '1d_tracks'), exist_ok=True)
                        # visualize 1D tracks as shaded plots
                        fig, axs = plt.subplots(len(pl_module.hparams.output_features), 1, figsize=(10, len(pl_module.hparams.output_features) * 2))
                        if len(pl_module.hparams.output_features) == 1:
                            axs = [axs]
                        colors = ['blue', 'orange', 'green', 'red', 'purple', 'brown', 'pink', 'gray']
                        for i, track_name in enumerate(pl_module.hparams.output_features):
                            pred_1d = pred_1d_tracks[track_name][0].detach().cpu().numpy()
                            axs[i].plot(pred_1d, color=colors[i % len(colors)])
                            axs[i].fill_between(range(len(pred_1d)), pred_1d, color=colors[i % len(colors)], alpha=0.5)
                            axs[i].set_title(track_name)
                            axs[i].set_xticks([])
...
def init_parser():
  ...
  parser.add_argument('--target-feature-size', dest='target_1d_size', type=int, default=4096,
...
class TrainModule(pl.LightningModule):
    ...
    def process_batch(self, batch, target_mat, target_1d_tracks=None):
        outputs = self(batch)

        pred_hic = outputs.get('hic')
        loss_hic = self.criterion(pred_hic, target_mat)
